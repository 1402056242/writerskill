...
def count_single_chapter(chapter_num, chapter_dir):
    ...
    if words < 1800:
        shortage = 1800 - words
        print(f"⚠️ 字数不足：还需 {shortage} 字才能达到最低标准 (1800字)")
    elif words > 3000:
        excess = words - 3000
        print(f"✅ 字数充足：超出建议上限 {excess} 字 (可接受)")
    else:
        print(f"✅ 字数达标：符合标准范围 (1800-3000字)")
...
def count_all_chapters(chapter_dir):
    ...
    ranges = [
        (0, 1500, "< 1500"),
        (1500, 1800, "1500-1800"),   # 明确标出不达标区间
        (1800, 2400, "1800-2400"),   # 达标中段
        (2400, 3000, "2400-3000"),   # 达标高段
        (3000, float('inf'), "> 3000")
    ]
    ...
    print()
    print("=== 字数不足章节 (< 1800 字) ===")
    insufficient = [(i, w) for i, w in chapter_stats if 0 < w < 1800]
    
    if insufficient:
        for i, w in insufficient:
            print(f"⚠️  第{i:3d}章: {w:5d} 字 (不足 {1800-w} 字)")
    else:
        print("✅ 所有章节字数均达标")
